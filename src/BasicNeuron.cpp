...
GLuint BasicNeuron::vertexbuffer = GLuint(0);
const GLfloat * BasicNeuron::g_buffer_data = BasicNeuron::generateBufferData();
...
void BasicNeuron::display(  )
{
	
}

void BasicNeuron::displayInformationInGUI(  )
{

}


GLfloat * BasicNeuron::generateBufferData()
{
	
	GLuint VertexArrayID;
	glGenVertexArrays(1, &VertexArrayID);
	glBindVertexArray(VertexArrayID);
	static GLfloat g_vertex_buffer_data[ BASIC_NEURON_VERTICES_AMOUNT ];
	static bool BNflagFirstVisit = true;
	
	if ( BNflagFirstVisit )
	{
		BNflagFirstVisit = false;
		for (unsigned int i = 0; i < BASIC_NEURON_TRIANGLES_AMOUNT; ++i)
		{
			g_vertex_buffer_data[ (i * 9) + 0 ] = 0;
			g_vertex_buffer_data[ (i * 9) + 1 ] = 0;
			g_vertex_buffer_data[ (i * 9) + 2 ] = 0;
			g_vertex_buffer_data[ (i * 9) + 3 ] =  cos ( i * 2.0f * M_PI / (double)BASIC_NEURON_TRIANGLES_AMOUNT );
			g_vertex_buffer_data[ (i * 9) + 4 ] = sin ( i * 2.0f * M_PI / (double)BASIC_NEURON_TRIANGLES_AMOUNT );
			g_vertex_buffer_data[ (i * 9) + 5 ] = 0;
			g_vertex_buffer_data[ (i * 9) + 6 ] =  cos ( (i + 1) * 2.0f * M_PI / (double)BASIC_NEURON_TRIANGLES_AMOUNT );
			g_vertex_buffer_data[ (i * 9) + 7 ] =  sin ( (i + 1) * 2.0f * M_PI / (double)BASIC_NEURON_TRIANGLES_AMOUNT );
			g_vertex_buffer_data[ (i * 9) + 8 ] = 0;
		}
	}
	
	static GLuint vertexbuffer_aux;
	glGenBuffers(1, & vertexbuffer_aux);
	glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer_aux);
	BasicNeuron::vertexbuffer = vertexbuffer_aux;
